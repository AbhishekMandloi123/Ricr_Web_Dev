...
const Currency = () => {
  ...
  const Convert = async () => {
    if (!from || !to || !fromAmt || parseFloat(fromAmt) <= 0) {
      toast.error("Please fill all fields with valid amount");
      return;
    }
    try {
      ...
      setToAmt(parseFloat(fromAmt)*res.data[from.split(" ")[0].toLowerCase()][to.split(" ")[0].toLowerCase()]);
